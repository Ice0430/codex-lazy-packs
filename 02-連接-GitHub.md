...
## 2026-05-05 新電腦同步踩坑紀錄

在新電腦重跑本章時，請把 GitHub CLI 和 Codex Desktop GitHub connector 分開驗證。兩者不是同一個登入狀態。
...
gh version 2.91.0
Logged in to github.com account Ice0430
Git operations protocol: https
Token scopes include: repo, workflow
```

若 `gh auth status` 顯示未登入，重跑：

```powershell
gh auth login --web --git-protocol https
```

Git 全域作者若尚未設定，可用 GitHub noreply email，避免公開私人 email：

```powershell
git config --global user.name "Ice0430"
git config --global user.email "214752600+Ice0430@users.noreply.github.com"
```

### connector 私有 repo 權限可能延遲或不一致

這次遇到的狀況：

- `gh repo list Ice0430 --limit 100` 可以看到 private repos。
- `gh repo view Ice0430/2026DataBase` 可以看到 private repo。
- `gh repo view Ice0430/2026-manmade-stone-translucency-data` 可以看到 private repo。
- Codex Desktop GitHub connector 已連到 `Ice0430`，也有 GitHub App installation。
- GitHub App repository access 改成 `All repositories` 後，connector 仍只回傳 public repos。
- 改成 `Only select repositories` 並選滿所有 repositories 後，connector 仍可能暫時只回傳 public repos。

因此判斷方式：

```powershell
gh repo list Ice0430 --limit 100 --json nameWithOwner,visibility,isPrivate
gh repo view Ice0430/2026DataBase --json nameWithOwner,visibility,defaultBranchRef
gh repo view Ice0430/2026-manmade-stone-translucency-data --json nameWithOwner,visibility,defaultBranchRef
```

如果上述 `gh` 指令正常，代表本機 GitHub 操作已可用，可以先用 GitHub CLI 做 clone / pull / push / repo 檢查。只有在需要 Codex connector 讀 issue、PR、repo metadata 時，才需要繼續處理 connector 私有 repo 可見性。

### connector 仍看不到 private repo 時

先不要卡住整個工作流程。可依序嘗試：

1. 到 GitHub App installation 頁面確認 repository access。
2. 嘗試 `Only select repositories`，手動選入全部需要的 repos。
3. 在 Codex Desktop 的 GitHub connector 重新連線。
4. 等待一段時間後再查 connector repo 清單。
5. 若仍未生效，先用 `gh` CLI 完成 GitHub 操作，待 issue / PR workflow 真的受影響時再回頭處理。
